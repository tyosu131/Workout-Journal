...
| 4. Continuous Delivery | Open | CI, Cloud Build image creation, the manual candidate/promotion/rollback runbook and P2B automated candidate smoke proof exist | Integrate the verified smoke into keyless GitHub OIDC/WIF CD; implement production Environment approval, promotion and post-deploy evidence, and close the Deploy-SA submission path under its separate gate | CD phase |
