...
**Must 4 remains OPEN:** C4B implements automatic **main-merge + required-CI-success
triggering** in source and verifies its contracts offline. Automatic CD/OIDC/WIF
runtime proof is **NOT YET**. The C3 manually dispatched success remains separate.
Closure still requires implementation merge, successful required CI, the automatic
CD run causally linked to that exact CI ID/attempt/source, Build/candidate/E2E/verify
success, Human production approval, paired promotion, post-deploy verification and
durable documentation closure. Existing bounded rollback evidence above remains
