...
- **P1B (Historical adoption checkpoint):** Manually bootstrapped and verified the dedicated state bucket, initialized the GCS backend, and imported all eight approved existing resources into remote state. The reviewed import apply reported `8 imported, 0 added, 0 changed, 0 destroyed`, and the post-import second plan reported no changes.
- **P1C-A (Historical initial disabled-provider checkpoint):** Applied and verified the additive, initially inert identity foundation: four prerequisite APIs, dedicated deploy/build Service Accounts, one federation-only pool, one disabled GitHub OIDC provider, and one repository-ID-scoped impersonation member. The reviewed apply reported `9 added, 0 changed, 0 destroyed`, and the post-apply plan reported no changes.
- **P1C-B (Historical IAM checkpoint):** Applied and verified exactly 13 additive operational IAM members for the approved deploy/build command path. The reviewed apply reported `13 added, 0 changed, 0 destroyed`, bringing remote state to 30 resources; the post-apply plan reported no changes.
...
Both policies reference the email channel and depend on Monitoring API ownership.
The uptime policy references the computed uptime ID. Missing data is INACTIVE,
not proof of service health. Backend 5xx includes tagged zero-traffic candidates.
Logging API stays externally enabled; there is no Cloud Run service resource,
...
It checks evaluated HCL and Terraform's effective dependency graph; it never
authenticates to GCP or executes import/apply. See [OBS verification](../../docs/verification.md#obs-bc-health-and-monitoring-implementation)
and the [inspection/recovery procedure](../../docs/cloud-run-deployment-runbook.md#observability).
